...
const AddDriverProfile = ({ history }) => {
    ...
    const saveUserInfo = (e) => {
        ...
        if (userInfo.email && validateEmail(userInfo.email)) {
            ServerAPI.post('/api/admin/user/email/duplicate', { email: userInfo.email }).then(res => {
                ServerAPI.post(`/api/admin/driver/profile`, { ...userInfo }).then(res => {
                    if (res.data.success === true) {
                        Swal.fire("User's profile was successfully created.")
                    } else {
                        Swal.fire("Failed to create driver's profile.")
                    }
                }).catch(e => {
                    Swal.fire("Failed to update driver's profile.")
                })
            }).catch(e => {
                    ...
                    <Form onSubmit={ saveUserInfo}>
                        <Row >
                            <Col lg="12">
                                <Card>
                                    <CardHeader>
                                        <h4 className="py-2 mt-3 mb-0"> Driver Details</h4>
                                    </CardHeader>
                                    <CardBody>
                                        ...
                                        <Row className="">
                                            <Col lg="12">
                                                <Row>
                                                    ...
                                                    <UserInput title={"Phone Number"} category="phone" user={userInfo.role} updateUserInfo={setUserData} size={4} />
                                                    <StaticInputItem title={"Account Type"} value={UserRole.DRIVER} size={4} />
                                                    <Col className="py-2">
